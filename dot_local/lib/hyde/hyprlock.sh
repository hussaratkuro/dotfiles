...
fn_mpris() {
    ...
    if [[ "${player_status}" == "Playing" || "${player_status}" == "Paused" ]]; then
        playerctl -p "${player}" metadata --format "$(mpris_icon "${player}")   {{xesam:artist}} - {{xesam:title}}"
        mpris_thumb "${player}"
    else
        if [ -f "$HOME/.face.icon" ]; then
...
mpris_thumb() { # Generate thumbnail for mpris
    local player="${1:-""}"
    local player_status="$(playerctl -p "${player}" status 2>/dev/null)"
    
    if [[ "${player_status}" == "Playing" || "${player_status}" == "Paused" ]]; then
        artUrl=$(playerctl -p "${player}" metadata --format '{{mpris:artUrl}}')
        [ "${artUrl}" == "$(cat "${THUMB}".lnk 2>/dev/null)" ] && [ -f "${THUMB}".png ] && exit 0
